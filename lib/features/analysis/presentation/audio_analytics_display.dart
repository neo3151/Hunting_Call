  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        if (analysis.topSpeciesMatches.isNotEmpty) ...[
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '${analysis.topSpeciesMatches.keys.first}: ${analysis.topSpeciesMatches.values.first.toStringAsFixed(0)}% 🔥',
                ...
                if (analysis.topSpeciesMatches.length > 1) ...[
                  const SizedBox(height: 8),
                  Text(
                    'Also heard: ${analysis.topSpeciesMatches.keys.elementAt(1)} (${analysis.topSpeciesMatches.values.elementAt(1).toStringAsFixed(0)}%)',
        ...
        _metricGrid([
          AnalyticsMetricCard(
              label: 'Dominant Frequency', value: analysis.dominantFrequencyHz, unit: 'Hz'),
          AnalyticsMetricCard(
              label: 'Average Frequency', value: analysis.averageFrequencyHz, unit: 'Hz'),
          AnalyticsMetricCard(label: 'Pitch Stability', value: analysis.pitchStability, unit: '%'),
        ]),
        if (analysis.pitchTrack.isNotEmpty) ...[
          const SizedBox(height: 12),
          Text('Pitch Development', style: GoogleFonts.lato(color: Colors.white70, fontSize: 12)),
          const SizedBox(height: 8),
        ...
        _metricGrid([
          AnalyticsMetricCard(
              label: 'Average Volume', value: analysis.averageVolume * 100, unit: '%'),
        ...
        _metricGrid([
          ...
          AnalyticsMetricCard(
              label: 'Harmonic Richness', value: analysis.harmonicRichness, unit: '%'),
        ...
        _metricGrid([
          ...
          AnalyticsMetricCard(
              label: 'Active Duration', value: analysis.activeDurationSec, unit: 's'),
          AnalyticsMetricCard(
              label: 'Silence Duration', value: analysis.silenceDurationSec, unit: 's'),
        ]),
        if (analysis.isPulsedCall) ...[
          ...
          _metricGrid([
            ...
            AnalyticsMetricCard(
                label: 'Pulses Detected', value: analysis.pulseTimes.length.toDouble(), unit: ''),
            AnalyticsMetricCard(
                label: 'Rhythm Regularity', value: analysis.rhythmRegularity, unit: '%'),
          ]),
        ],
      ],
    );

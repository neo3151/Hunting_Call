...
def main():
    ...
    # Upload APK
    apk_path = os.path.join(script_dir, "..", "build", "app", "outputs", "flutter-apk", "OUTCALL-v2.1.0-debug-2026-03-25.apk")
    ...
    existing = service.files().list(
        q=f"name='OUTCALL-v2.1.0-debug-2026-03-25.apk' and '{folder_id}' in parents and trashed=false",
        fields="files(id, name)"
    ).execute().get("files", [])
    ...
    if existing:
        ...
        # Create new file
        file_meta = {
            "name": "OUTCALL-v2.1.0-debug-2026-03-25.apk",
            "parents": [folder_id]
        }

...
export class Director extends Agent {
    ...
    private interactionLogs: string[] = [];
    private highlights: string[] = [];
    private efficiencyMetrics: Map<string, { totalTime: number, impactScore: number }> = new Map();
    ...
    async runAutonomously(maxCycles: number = 10) {
...
4. Exit on queue depletion or cycle limit.`);
    ...
    private addInteraction(message: string) {
        this.interactionLogs.push(`[${new Date().toISOString()}] ${message}`);
    }

    private addHighlight(agent: string, role: string, result: string) {
        this.highlights.push(`${agent} (${role}): "${result}"`);
    }

    private updateEfficiency(agent: string, timeTaken: number, impact: number) {
    ...
    getWorkplaceReview(): string {
        ...
        const incidents = this.interactionLogs.filter(log =>
            log.includes('CRITICAL') || log.includes('REJECTED') || log.includes('failed')
        );
    ...
    async processTask(task: AgentTask, allTasks?: AgentTask[]): Promise<AgentTask> {
        // Ensure missionId exists (default to task.id if new mission)
        if (!task.missionId) task.missionId = task.id;
...
4. Orchestrate quality assurance and final approval.`, task.missionId);
            ...
            try {
                ...
                while (attempts < maxAttempts) {
                    ...
                    if (currentTask.status === 'completed') {
                        ...
                        if (currentTask.status === 'failed') break;
                    }
                }
    ...
    private async executeWithRetry(worker: Agent, task: AgentTask, maxRetries: number = 2): Promise<AgentTask> {
        let retries = 0;
        worker.isBusy = true;
        try {
    ...
    private routeTask(task: AgentTask): Agent | undefined {
        const desc = task.description.toLowerCase();
        let bestWorker: Agent | undefined = undefined;
        let maxScore = -1;

        for (const worker of this.workers) {
            // Do not route to busy agents or QA agents
            if (worker.role === AgentRole.QA || worker.isBusy) continue;
            ...
            if (worker.role === AgentRole.SCOUT && (desc.includes('explore') || desc.includes('scan') || desc.includes('discovery') || desc.includes('lateral'))) score += 20;
            if (worker.role === AgentRole.RESEARCHER && (desc.includes('research') || desc.includes('analysis') || desc.includes('source') || desc.includes('intel') || desc.includes('gathering') || desc.includes('audit') || desc.includes('security'))) score += 15;
            if (worker.role === AgentRole.WORKER && (desc.includes('build') || desc.includes('implement'))) score += 5;
            if (worker.role === AgentRole.ARCHITECT && (desc.includes('design') || desc.includes('architecture'))) score += 15;
            if (worker.role === AgentRole.OPTIMIZER && (desc.includes('optimize') || desc.includes('efficiency'))) score += 15;
            if (worker.role === AgentRole.SECURITY && (desc.includes('audit') || desc.includes('security'))) score += 15;
            if (worker.role === AgentRole.CODE_ANALYZER && (desc.includes('code') || desc.includes('scan') || desc.includes('analyze') || desc.includes('module') || desc.includes('file'))) score += 25;
        ...
        return this.workers.find(w => !w.isBusy && w.role !== AgentRole.QA);
    }

    private hasCircularDependency(task: AgentTask, allTasks: AgentTask[]): boolean {
        ...
        const findCycle = (id: string): boolean => {
            ...
            if (t && t.dependencies) {
                for (const depId of t.dependencies) {
                    if (findCycle(depId)) return true;
                }
            }

...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockLoggerService.recordError(any(), any(), reason: any(named: 'reason')))
        .thenReturn(null);
  ...
  group('RecordingController / RecordingNotifier Tests', () {
    ...
    test('startRecordingWithCountdown enters countdown and then recording on success', () async {
      ...
      // 2. Act
      final future =
          container.read(recordingNotifierProvider.notifier).startRecordingWithCountdown();
      ...
      verify(() => mockLoggerService.log(any(that: contains('Recording actually started'))))
          .called(1);
    });
    ...
    test('startRecordingWithCountdown enters error state if use case fails', () async {
      ...
      verify(() =>
              mockLoggerService.recordError(failure, any(), reason: 'Failed to start recording'))
          .called(1);
    });
    ...
    test('stopRecording returns audio path and goes idle on success', () async {
      ...
      when(() => mockStopUseCase.execute())
          .thenAnswer((_) async => const Right('/tmp/test_audio.wav'));
      ...
      // Check log
      verify(() => mockLoggerService.log(
          any(that: contains('Recording stopped successfully: /tmp/test_audio.wav')))).called(1);
    });
    ...
    test('stopRecording captures and logs failure', () async {
      ...
      // Check log
      verify(() =>
              mockLoggerService.recordError(failure, any(), reason: 'Failed to stop recording'))
          .called(1);
    });

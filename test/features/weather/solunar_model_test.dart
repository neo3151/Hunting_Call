...
void main() {
  ...
  group('SolunarModel.fromOpenMeteo', () {
    test('parses sunrise and sunset', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(), date);
      expect(model.sunrise, DateTime(2025, 3, 1, 6, 30));
      expect(model.sunset, DateTime(2025, 3, 1, 18, 30));
    });

    test('parses moon illumination', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonIllumination: 75.5), date);
      expect(model.moonIllumination, 75.5);
    });

    test('moon phase 0 → New Moon', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.0), date);
      expect(model.moonPhaseName, 'New Moon');
    });

    test('moon phase 1 → New Moon', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 1.0), date);
      expect(model.moonPhaseName, 'New Moon');
    });

    test('moon phase 0.1 → Waxing Crescent', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.1), date);
      expect(model.moonPhaseName, 'Waxing Crescent');
    });

    test('moon phase 0.25 → First Quarter', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.25), date);
      expect(model.moonPhaseName, 'First Quarter');
    });

    test('moon phase 0.4 → Waxing Gibbous', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.4), date);
      expect(model.moonPhaseName, 'Waxing Gibbous');
    });

    test('moon phase 0.5 → Full Moon', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.5), date);
      expect(model.moonPhaseName, 'Full Moon');
    });

    test('moon phase 0.6 → Waning Gibbous', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.6), date);
      expect(model.moonPhaseName, 'Waning Gibbous');
    });

    test('moon phase 0.75 → Last Quarter', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.75), date);
      expect(model.moonPhaseName, 'Last Quarter');
    });

    test('moon phase 0.9 → Waning Crescent', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonPhase: 0.9), date);
      expect(model.moonPhaseName, 'Waning Crescent');
    });

    test('rating 5 for high illumination (>90)', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonIllumination: 95.0, moonPhase: 0.5), date);
      expect(model.overallRating, 5);
    });

    test('rating 5 for low illumination (<10)', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonIllumination: 5.0, moonPhase: 0.0), date);
      expect(model.overallRating, 5);
    });

    test('rating 4 for illumination 70-90', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonIllumination: 80.0, moonPhase: 0.4), date);
      expect(model.overallRating, 4);
    });

    test('rating 4 for illumination 10-30', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonIllumination: 20.0, moonPhase: 0.1), date);
      expect(model.overallRating, 4);
    });

    test('rating 3 for mid illumination', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonIllumination: 50.0, moonPhase: 0.25), date);
    ...
    test('major period is 2 hours centered on midpoint', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(
    ...
    test('minor periods are 1 hour centered on event', () {
      ...
      final moonriseMinor = model.minorPeriods.where((p) => 
        p.start == DateTime(2025, 3, 1, 9, 30)).toList();
    ...
    test('no major periods when moonrise is null', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonrise: null), date);
      expect(model.majorPeriods, isEmpty);
    });

    test('no major periods when moonset is null', () {
      final model = SolunarModel.fromOpenMeteo(_makeJson(moonset: null), date);
      expect(model.majorPeriods, isEmpty);
    });

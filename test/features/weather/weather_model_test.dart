...
void main() {
  group('WeatherModel.fromJson', () {
    ...
    test('parses all numeric fields correctly', () {
      final model = WeatherModel.fromJson(_makeJson(
        temperature: 32.7,
        windSpeed: 15.3,
        windDirection: 270.0,
        humidity: 42,
    ...
    test('WMO code 0 → Clear', () {
      final model = WeatherModel.fromJson(_makeJson(weatherCode: 0));
    ...
    test('WMO code 1-3 → Partly Cloudy', () {
      for (final code in [1, 2, 3]) {
        final model = WeatherModel.fromJson(_makeJson(weatherCode: code));
    ...
    test('unknown WMO code → Unknown / default icon', () {
      final model = WeatherModel.fromJson(_makeJson(weatherCode: 200));
      expect(model.condition, 'Unknown');
      expect(model.iconCode, '01d');
    });

    test('gap WMO codes (4-44) → Unknown', () {
      final model = WeatherModel.fromJson(_makeJson(weatherCode: 10));
      expect(model.condition, 'Unknown');
    });
    ...
    test('negative temperature', () {
      final model = WeatherModel.fromJson(_makeJson(temperature: -15.3));
      expect(model.temperature, -15.3);
    });

    test('zero wind speed', () {
      final model = WeatherModel.fromJson(_makeJson(windSpeed: 0.0));
